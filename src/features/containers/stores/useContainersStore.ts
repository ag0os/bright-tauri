...
export const useContainersStore = create<ContainersState>((set, get) => ({
  ...
  loadContainers: async (universeId) => {
    ...
    try {
      const containers = await api.listContainers(universeId);
  ...
  createContainer: async (input) => {
    set({ isLoading: true, error: null });
    try {
      const container = await api.createContainer(input);
      set((state) => ({
  ...
  updateContainer: async (id, input) => {
    set({ isLoading: true, error: null });
    try {
      const container = await api.updateContainer(id, input);
      set((state) => ({
  ...
  deleteContainer: async (id) => {
    ...
    try {
      ...
      const deletedIds = await api.deleteContainer(id);
  ...
  loadContainerChildren: async (containerId) => {
    ...
    try {
      const children = await api.listContainerChildren(containerId);
      set((state) => {
  ...
  reorderChildren: async (containerId, containerIds, storyIds) => {
    ...
    set({ error: null });
    try {
      await api.reorderContainerChildren(containerId, containerIds, storyIds);
