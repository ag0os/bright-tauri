...
export const useElementsStore = create<ElementsState>((set, get) => ({
  ...
  loadElements: async (universeId) => {
    ...
    try {
      const elements = await api.listElementsByUniverse(universeId);
  ...
  createElement: async (input) => {
    set({ isLoading: true, error: null });
    try {
      const element = await api.createElement(input);
      set((state) => ({
  ...
  getElement: async (id) => {
    set({ isLoading: true, error: null });
    try {
      const element = await api.getElement(id);
      set({ isLoading: false });
      return element;
    } catch (error) {
  ...
  updateElement: async (id, input) => {
    set({ isLoading: true, error: null });
    try {
      const element = await api.updateElement(id, input);
      set((state) => ({
